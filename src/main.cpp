...
std::optional<json> httpGetJson(const std::string& url, const std::string& authHeader = "") { std::string cmd = "curl -sS --max-time 2 " + clawforge::util::Shell::quote(url); if (!authHeader.empty()) cmd += " -H " + clawforge::util::Shell::quote(authHeader); const auto res = clawforge::util::Shell::run(cmd); if (res.exitCode != 0) return std::nullopt; auto parsed = json::parse(res.output, nullptr, false); if (parsed.is_discarded()) return std::nullopt; return parsed; }
...
int runConfigSet(const std::string& configPath, const std::string& key, const std::string& value) {
  auto j = loadJsonFile(configPath);
  if (key == "model.current") j["modelsConfig"]["routing"]["current"] = value;
...
int main(int argc, char** argv) {
  try {
    ...
    if (command == "cron" && pos.size() >= 2 && pos[1] == "list") return runCronList(configPath);
    ...
    std::set<std::string> compatTop = {"setup","onboard","configure","dashboard","reset","uninstall","update","message","agent","agents","acp","gateway","logs","system","memory","nodes","devices","node","approvals","sandbox","browser","dns","docs","hooks","webhooks","plugins","channels","security","skills","tui"};
